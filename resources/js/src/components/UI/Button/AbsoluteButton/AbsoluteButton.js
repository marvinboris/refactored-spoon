...
const absoluteButton = ({ children, color, icon, iconColor = 'reset', className = '', size = "", pill = false }) =>
    <Button color={color} size={size} className={"text-500 text-montserrat-alt " + (pill ? ' rounded-pill ' : '') + className}>
        <span className="position-relative px-4 d-inline-flex align-items-center text-500">
