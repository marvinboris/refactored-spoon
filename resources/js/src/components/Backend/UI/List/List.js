    ...
    render() {
        ...
        return (
            <Col xs={xs} sm={sm} md={md} lg={lg} xl={xl} className={outerClassName}>
                ...
                <div className={`rounded-4 d-flex justify-content-between align-items-center mb-5 mt-3 py-4 px-4 text-large bg-pink-10 ${className}`}>
                    <span className="d-inline-flex align-items-center text-700 text-scarlet">{icon ? <FontAwesomeIcon fixedWidth className="mr-2" icon={icon} size="lg" /> : null}<span className="text-dark">{title}</span></span>

                    {add ? <Link to={link}>
                        <Button color="pink" size="lg" className="rounded-2">
                ...
                <div className={"d-flex flex-column " + (dark ? "text-light " : " ") + className} style={style}>
                    <div className="p-4 border-bottom border-soft text-pink text-700 position-relative">
