...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'employees');
        ...
        const data = employees.map(employee => {
            return updateObject(employee, {
                ...
                action: <div className="text-center">
                    <Link to={`/user/employees/${employee.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/employees/${employee.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(employee.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(employees)} get={this.props.get} total={total} bordered add={add} link="/user/employees/add" icon={faUserTie} title={index} className="shadow-sm"
                        fields={[
                            { name, key: 'name' },
