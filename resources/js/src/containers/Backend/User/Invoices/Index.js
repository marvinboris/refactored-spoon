...
class Index extends Component {
    ...
    render() {
        ...
        const data = invoices.map(invoice => {
            return updateObject(invoice, {
                ...
                action: <div className="text-center">
                    <Link to={`/user/invoices/${invoice.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/invoices/${invoice.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(invoice.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(invoices)} get={this.props.get} total={total} bordered add={add} link="/user/invoices/add" icon={faFileInvoice} title={index} className="shadow-sm"
                        fields={[
                            { name: no, key: 'no' },
                            { name: date, key: 'date' },
