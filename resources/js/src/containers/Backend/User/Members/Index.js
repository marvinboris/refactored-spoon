...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'members');
        ...
        const data = members.map(member => {
            return updateObject(member, {
                ...
                action: <div className="text-center">
                    <Link to={`/user/members/${member.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/members/${member.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(member.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(members)} get={this.props.get} total={total} bordered add={add} link="/user/members/add" icon={faUserTie} title={index} className="shadow-sm"
                        fields={[
                            { name, key: 'name' },
                            { name: job, key: 'job' },
