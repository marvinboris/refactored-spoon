        ...
        const data = tasks.map(task => {
            return updateObject(task, {
                ...
                action: <div className="text-center">
                    ...
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/tasks/${task.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(task.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
