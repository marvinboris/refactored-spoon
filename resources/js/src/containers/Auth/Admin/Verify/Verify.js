...
export class Verify extends Component {
    ...
    render() {
        ...
        titleContent = <>
            {enter} <span className="text-yellow">{verification_code}</span>
        </>;
        ...
        formContent = <>
            ...
            <FormGroup className="ml-2 mb-5 mt-4">
                <p className="text-dark text-right">{didnt_receive_code}? <strong className="text-yellow" style={{ cursor: 'pointer' }} onClick={() => onResendCode(hash)}>{resend}</strong></p>
            </FormGroup>

            <BetweenButton color="yellow" size="lg" className="py-3 px-4 btn-block" icon={faSignInAlt}>{continue_}</BetweenButton>
        </>;
